...
#define USE_GPIOS_NUM 8   /* number of GPIOs that we are going to allocate */
...
extern int gpio_lkm_write_pin(int num, bool value){
    if ((num >= 0) && (num < USE_GPIOS_NUM) && (gpio_lkm_devp[num]->pin != null)){
        /* set direction output and low level */
        gpio_direction_output(gpio_lkm_devp[num]->pin.gpio, value ? high : low);
        /* store state in device struct */
        if (gpio_lkm_devp[num]->dir != out){
            printk(KERN_INFO " Set GPIO%d direction: ouput\n", gpio_lkm_devp[num]->pin.gpio);
            gpio_lkm_devp[num]->dir = out;
        }

        printk(KERN_INFO " Set GPIO%d state: ouput\n", gpio_lkm_devp[num]->pin.gpio);
        gpio_lkm_devp[num]->state = value ? high : low;
        return 0;
    }

    printk(KERN_ERR "[GPIO_LKM] - Invalid num value\n");
    return -1;
}

extern int gpio_lkm_read_pin(int num){
    if ((num >= 0) && (num < USE_GPIOS_NUM) && (gpio_lkm_devp[num]->pin != null)){
        return gpio_get_value(gpio_lkm_devp[num]->pin.gpio);
    }
...
EXPORT_SYMBOL(gpio_lkm_write_pin);
EXPORT_SYMBOL(gpio_lkm_read_pin);
...
static int __init gpio_lkm_init(void)
{
    ...
    if ((gpio_lkm_class = class_create( THIS_MODULE, DEVICE_NAME)) == NULL)
    {
        ...
        /* clean up what was done in case of failure
         */
